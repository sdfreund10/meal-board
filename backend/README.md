...
uv sync
cp .env.example .env
# Required: HOUSEHOLD_PIN, ADMIN_PASSWORD, SESSION_SECRET (≥32 chars)
...
- `POST /api/auth/elevate` `{ "password": "..." }` → admin access for 24 hours
- `GET /api/auth/me` → `{ "authenticated": bool, "admin_access": bool }`
- `POST /api/auth/logout` → clear cookie

Recipe catalog reads are public. Week, grocery, and tag reads require a household
session. All recipe, tag, and week mutations require elevated admin access.
